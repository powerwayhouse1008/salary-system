      ...
      <form action={saveFormula} className="grid gap-4 rounded-lg border border-line bg-white p-4 lg:grid-cols-[1fr_320px]">
        ...
        <div className="space-y-3">
          <label className="field">
            名称
            <input name="name" defaultValue={active.name} readOnly={!isAdmin} />
          </label>
          <label className="field">
            合計
            <textarea name="formula_total" rows={2} defaultValue={expressions[0]} readOnly={!isAdmin} />
          </label>
          <label className="field">
            控除合計
            <textarea name="formula_deduction" rows={2} defaultValue={expressions[1]} readOnly={!isAdmin} />
          </label>
          <label className="field">
            振込金額
            <textarea name="formula_transfer" rows={2} defaultValue={expressions[2]} readOnly={!isAdmin} />
          </label>
          <label className="field">
            残り金額
            <textarea name="formula_remaining" rows={2} defaultValue={expressions[3]} readOnly={!isAdmin} />
          </label>
          <label className="flex items-center gap-2 text-sm font-semibold">
            <input name="is_default" type="checkbox" defaultChecked={active.is_default} disabled={!isAdmin} className="h-4 w-4" />
            標準にする
          </label>
          {isAdmin ? (
        ...
        <table className="data-table">
          ...
          <tbody>
            {formulas.map((formula) => (
              <tr key={formula.id}>
                <td>{formula.name}</td>
                <td>{formula.is_default ? "はい" : ""}</td>
                <td>{formula.formula_total}</td>
              </tr>
            ))}
          </tbody>
        </table>
